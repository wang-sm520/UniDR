...
def _package_root(name: str) -> Path:
    """Record the imported owner, including editable bundled distributions."""
    spec = find_spec(name)
    if spec is None or spec.origin is None:
        raise ImportError(f"cannot locate the active {name} package")
...
def build_sources(cfg: DictConfig, root: Path) -> tuple[list[SourceSpec], list[dict]]:
    ...
    if cfg.unidr.get("adaptive") is not None and cfg.unidr.adaptive.probe.horizon != 224:
        raise ValueError("G1 flip probes require the complete 224-step reference opportunity")
    ...
    for index, source in enumerate(cfg.unidr.sources):
        ...
        sources.append(
            SourceSpec(
                source.name,
                probe_env_factory(source.name)
                if cfg.unidr.get("adaptive") is not None
                else registry_env_factory("G1FlipTracking", source.name),
...
def build_manifest(cfg: DictConfig, root: Path, snapshots: list[dict]) -> dict[str, Any]:
    ...
    if OmegaConf.select(cfg, "unidr.adaptive") is not None:
        behavior["adaptive"] = OmegaConf.to_container(cfg.unidr.adaptive, resolve=True)
    ...
    return {
        ...
        **behavior,
        "repositories": {
            ...
            for name, path in {
                "unilab": root,
                "uni_rl": _package_root("uni_rl"),
                "unisim": _package_root("unisim"),
            }.items()
        },
        "packages": {
...
def run_synchronous_training(cfg: DictConfig, root: Path) -> None:
    ...
    try:
        ...
        runner = SynchronousPPORunner(
            ...
            adaptive=cast(dict, OmegaConf.to_container(cfg.unidr.adaptive, resolve=True))
            if cfg.unidr.get("adaptive") is not None
            else None,
        )
        if cfg.algo.resume:
