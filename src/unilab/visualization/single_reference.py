"""Fixed-policy holdout with phase-matched reference rendering."""
...
from uni_rl.logging.synchronous_report import audit_run as audit_joint_run
...
def preflight(
    ...
    *,
    ...
    selected_checkpoint: bool = False,
) -> HoldoutPlan:
    ...
    if not selected_checkpoint and checkpoint.name != f"model_{expected_iterations - 1}.pt":
        raise ValueError("Only the explicitly requested final checkpoint is allowed")
    if selected_checkpoint:
        from uni_rl.logging.checkpoint_audit import audit_single_checkpoint

        audit = audit_single_checkpoint(
            checkpoint.parent,
    ...
    audited_hash = (
        audit["checkpoint_sha256"] if selected_checkpoint else audit["final_checkpoint_sha256"]
    )
    if audited_hash != checkpoint_hash:
        raise ValueError("Checkpoint changed after its native training audit")
    models = _models(target)
    completed = audit["completed_iterations"] if selected_checkpoint else expected_iterations
    transitions = completed * expected_num_envs * 24
...
def record_reference(
    ...
    *,
    ...
    expected_iterations: int = 20000,
    expected_num_envs: int = 1024,
    failure_tail_seconds: float = 0.0,
) -> dict[str, Any]:
    ...
    return _record_plan(plan, output, root=root, failure_tail_seconds=failure_tail_seconds)


def record_joint_reference(
    checkpoint: Path, output: Path, *, root: Path, expected_iterations: int = 5000
) -> dict[str, Any]:
    """Use the same reference renderer for the fully audited fresh joint final."""
    audit = audit_joint_run(
        checkpoint.parent, expected_iterations=expected_iterations, num_envs=1024
    )
    plan = joint_preflight(checkpoint, root, expected_iterations=expected_iterations)
    if (
        audit["final_checkpoint_sha256"] != plan.checkpoint_sha256
        or Path(audit["final_checkpoint"]).resolve() != plan.checkpoint
        or audit["generations"] != [0]
        or len(audit["history"]) != 1
        or audit["history"][0]["resume_checkpoint"] is not None
        or plan.metadata["planned_iterations"] != expected_iterations
    ):
        raise ValueError("Joint reference playback requires the complete fresh final run")
    plan = replace(
        plan,
        metadata={
            **plan.metadata,
            ...
            "training_envs_per_source": 1024,
            "optimizer_steps": audit["optimizer_steps"],
            "training_audit": audit,
        },
    )
    return _record_plan(plan, output, root=root)
...
def _record_plan(
    ...
    *,
    ...
    if not np.isfinite(failure_tail_seconds) or not 0 <= failure_tail_seconds <= 10:
        raise ValueError("Failure-tail seconds must be finite and between 0 and 10")
    tail_steps = round(failure_tail_seconds * FPS)
    if not np.isclose(tail_steps, failure_tail_seconds * FPS):
        raise ValueError("Failure-tail duration must be a whole number of control steps")
    ...
    if tail_steps:
        ...
        (output / "playback_overrides.json").write_text(
            json.dumps(
                dict(
                    mode="diagnostic_failure_tail",
                    native_config="mujoco_config.json",
                    failure_tail_steps=tail_steps,
                    reference_end="native cycles before first done; then hold original final frame",
                    termination="observe native terms; delay reset, true termination wins",
                    policy="continue frozen trained actor on live observations",
                    original_holdout_metrics_unchanged=True,
                ),
                indent=2,
            )
            + "\n"
    ...
    with closing(registry_env_factory("G1FlipTracking", "mujoco")(1, override)) as env:
        continuation = None
        if tail_steps:
            continuation = FailureTailTerminations(env, tail_steps)
        ...
        previous_actions = np.zeros((1, 29), dtype=np.float32)
        with torch.inference_mode(), (output / "telemetry.jsonl").open("x") as stream:
            for step in range(STEPS):
                ...
                row.update(
                    ...
                    action_delta_rms=float(np.sqrt(np.mean((actions - previous_actions) ** 2))),
                )
                previous_actions = np.zeros_like(actions) if row["reset"] else actions.copy()
                if continuation is not None:
                    row.update(
                        native_terminated=continuation.native_terminated,
                        native_truncated=continuation.native_truncated,
                        first_native_failure=continuation.steps_after_failure == 0,
                        steps_after_failure=continuation.steps_after_failure,
                    )
                    if continuation.first_done is not None:
                        row["landing_candidate"] = False
    ...
    video = _render_recording(states, refs, paths, output) if render_video else None
    ...
    result = dict(
        ...
        **plan.metadata,
        ...
        format=validate_video(video) if video else None,
        video_sha256=sha256(video.read_bytes()).hexdigest() if video else None,
        text_overlays=False,
        reference_rgba=RGBA,
    ...
    if continuation is not None:
        result.update(
            playback_mode="diagnostic_failure_tail",
            failure_tail_steps=tail_steps,
            failure_tail_seconds=tail_steps / FPS,
            first_native_failure_steps=[
                row["step"] for row in telemetry.rows if row["first_native_failure"]
            ],
            delayed_reset_steps=[row["step"] for row in telemetry.rows if row["reset"]],
            reference_end="native cycles before first done; then hold original final frame",
            holdout_scoring=False,
            incomplete_final_tail=(
                telemetry.rows[-1]["steps_after_failure"] is not None
                and telemetry.rows[-1]["steps_after_failure"] < tail_steps
            ),
        )
...
def _render_recording(states: np.ndarray, refs: np.ndarray, paths: list[str], output: Path) -> Path:
