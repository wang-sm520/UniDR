...
def test_formal_5000_budget_matches_fresh_configured_intent(tmp_path):
    run = fixture_run(tmp_path / "run", 5000, 1024)
    result = audit_run(run, expected_iterations=5000)
    assert result["scope"] == "formal_training_budget"
    assert result["total_transitions"] == 491520000 and result["optimizer_steps"] == 100000
    prefix = audit_run(run, expected_iterations=5000, final_checkpoint=run / "model_4999.pt")
    assert prefix["scope"] == "explicit_checkpoint_prefix"
...
def test_explicit_first_checkpoint_audits_budget_without_claiming_parameter_change(tmp_path):
    run = fixture_run(tmp_path / "run", iterations=3)
    selected = run / "model_0.pt"
    before = selected.read_bytes()
    result = audit_run(run, expected_iterations=1, num_envs=2, final_checkpoint=selected)
    assert result["scope"] == "explicit_checkpoint_prefix"
    assert result["configured_max_iterations"] == 3 and result["iterations"] == 1
    assert result["total_transitions"] == 192 and result["optimizer_steps"] == 20
    assert result["source_total_transitions"] == dict.fromkeys(SOURCE_ORDER, 48)
    assert result["parameters_changed_since_iteration_0"] == {"actor": False, "critic": False}
    assert "same checkpoint" in result["evidence_limit"]
    assert result["normalizer_sha256"] == result["iteration_0_normalizer_sha256"]
    assert result["history"][0]["through_iteration"] == 0
    assert result["history"][0]["excluded_metric_tail_bytes"] > 0
    assert selected.read_bytes() == before


@pytest.mark.parametrize("iterations", [0, -1, True, 1.0])
def test_explicit_first_checkpoint_rejects_invalid_iteration_budget(tmp_path, iterations):
    run = fixture_run(tmp_path / "run")
    with pytest.raises(ValueError, match="expected iterations"):
        audit_run(
            run, expected_iterations=iterations, num_envs=2, final_checkpoint=run / "model_0.pt"
        )


def test_full_run_still_requires_two_iterations(tmp_path):
    run = fixture_run(tmp_path / "run", iterations=1)
    with pytest.raises(ValueError, match="at least two"):
        audit_run(run, expected_iterations=1, num_envs=2)


@pytest.mark.parametrize("fault", ["counter", "adam", "count", "nan", "metric", "selection"])
def test_explicit_first_checkpoint_retains_learner_and_metric_validation(tmp_path, fault):
    run = fixture_run(tmp_path / "run")
    selected = run / "model_0.pt"
    saved = torch.load(selected, weights_only=False)
    if fault == "counter":
        saved["synchronous"]["total_transitions"] -= 1
    elif fault == "adam":
        ...
        rows[0]["epoch_samples"][0] -= 1
        path.write_text("\n".join(json.dumps(row) for row in rows) + "\n")
    torch.save(saved, selected)
    if fault == "selection":
        selected = run / "model_1.pt"
    with pytest.raises(ValueError):
        audit_run(run, expected_iterations=1, num_envs=2, final_checkpoint=selected)
...
def test_report_creates_json_and_standalone_curves(tmp_path, prefix):
    ...
    assert "timing" not in result and not (output / "timing.png").exists()


def add_timing(run, *, partial_history=False):
    path = run / "synchronous_metrics.jsonl"
    rows = [json.loads(line) for line in path.read_text().splitlines()]
    for row in rows[int(partial_history) :]:
        for index, name in enumerate(SOURCE_ORDER):
            row["sources"][name].update(
                env_step_seconds=0.1 * (index + 1),
                request_response_seconds=0.2 * (index + 1),
                barrier_wait_seconds=0.8 - 0.2 * (index + 1),
                timing_steps=24,
            )
    path.write_text("\n".join(json.dumps(row) for row in rows) + "\n")
    return path, rows
...
@pytest.mark.parametrize("partial_history", [False, True])
def test_source_timing_reports_coverage_and_single_shared_learner(tmp_path, partial_history):
    pytest.importorskip("matplotlib")
    run = fixture_run(tmp_path / "run")
    add_timing(run, partial_history=partial_history)
    result = report(run, tmp_path / "report", expected_iterations=2, num_envs=2)
    timing = result["timing"]
    count = 2 - int(partial_history)
    assert timing["timed_iterations"] == count
    assert timing["missing_iterations"] == int(partial_history)
    assert timing["first_iteration"] == int(partial_history) and timing["last_iteration"] == 1
    assert timing["global"]["learn_seconds"]["total"] == 0.5 * count
    assert timing["global"]["collect_seconds"]["mean"] == 2.0
    assert "learn_seconds" not in timing["sources"]["isaacsim"]
    for index, name in enumerate(SOURCE_ORDER):
        assert timing["sources"][name]["env_step_seconds"]["total"] == pytest.approx(
            0.1 * (index + 1) * count
        )
    assert (tmp_path / "report/timing.png").read_bytes().startswith(b"\x89PNG")
    assert (tmp_path / "report/timing.pdf").read_bytes().startswith(b"%PDF")
...
def test_bad_optional_source_timing_rejected_before_report_output(tmp_path, fault):
    run = fixture_run(tmp_path / "run")
    path, rows = add_timing(run)
    source = rows[-1]["sources"]["motrix"]
    if fault == "partial_field":
        source.pop("env_step_seconds")
        ...
        source["timing_steps"] = 23
    elif fault == "boolean":
        source["env_step_seconds"] = True
    elif fault == "nan":
        source["request_response_seconds"] = float("nan")
    elif fault == "negative":
        source["barrier_wait_seconds"] = -0.1
    elif fault == "worker_envelope":
        source["env_step_seconds"] = 0.9
    elif fault == "collect_envelope":
        source["barrier_wait_seconds"] = 2.0
    else:
        source["env_step_seconds"] = None
    path.write_text("\n".join(json.dumps(row) for row in rows) + "\n")
    with pytest.raises(ValueError):
        report(run, tmp_path / "report", expected_iterations=2, num_envs=2)
