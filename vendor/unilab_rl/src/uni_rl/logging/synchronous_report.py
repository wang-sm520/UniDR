...
SOURCE_TIMES = ("env_step_seconds", "request_response_seconds", "barrier_wait_seconds")


def _source_timing(row: dict) -> bool:
    """Historical rows may omit timing; an instrumented step must cover every source."""
    fields = {*SOURCE_TIMES, "timing_steps"}
    present = [fields.intersection(row["sources"][name]) for name in SOURCE_ORDER]
    if not any(present):
        return False
    _require(all(keys == fields for keys in present), "partial source timing schema")
    for name in SOURCE_ORDER:
        source = row["sources"][name]
        _integer(source["timing_steps"], 24, f"{name}/timing_steps")
        times = [_number(source[key], f"{name}/{key}") for key in SOURCE_TIMES]
        _require(min(times) >= 0, "negative source timing")
        _require(times[0] <= times[1] + 1e-6, "worker time exceeds request response time")
        _require(
            times[1] + times[2] <= row["collect_seconds"] + 1e-6,
            "source timing exceeds collection envelope",
        )
    return True
...
def _duration_summary(values: list[float]) -> dict:
    ...
    low = int(index)
    p95 = ordered[low] + (ordered[min(low + 1, len(values) - 1)] - ordered[low]) * (index - low)
    return dict(
        total=sum(values), mean=sum(values) / len(values), min=ordered[0], p95=p95, max=ordered[-1]
    )


def _timing_summary(rows: list[dict]) -> dict | None:
    timed = [row for row in rows if "timing_steps" in row["sources"][SOURCE_ORDER[0]]]
    if not timed:
        return None
    return {
        "units": "seconds",
        "timed_iterations": len(timed),
        "missing_iterations": len(rows) - len(timed),
        "first_iteration": timed[0]["iteration"],
        "last_iteration": timed[-1]["iteration"],
        "source_fields": {
            "env_step_seconds": "Worker wall time inside env.step, summed over 24 control steps.",
            "request_response_seconds": "Parent send/receive wall time, excluding thread-pool queue, summed over 24 steps.",
            "barrier_wait_seconds": "Time from this response being ready to the last source response, summed over 24 steps.",
        },
        "interpretation": "Source durations overlap and must not be summed as elapsed training time. PPO learn_seconds belongs to the one shared learner; global summaries cover timed iterations only.",
        "global": {
            key: _duration_summary([row[key] for row in timed])
            for key in ("collect_seconds", "learn_seconds")
        },
        "sources": {
            name: {
                key: _duration_summary([row["sources"][name][key] for row in timed])
                for key in SOURCE_TIMES
            }
            for name in SOURCE_ORDER
        },
    }
...
def _audit(
    ...
    _require(
        type(expected_iterations) is int
        and (
            expected_iterations >= 2 or (expected_iterations == 1 and final_checkpoint is not None)
        ),
        "at least two expected iterations required unless explicitly selecting the first checkpoint",
    )
    ...
    result = {
        ...
        if num_envs == 1024 and expected_iterations in (5000, 10000)
    ...
    if final_checkpoint is not None:
        ...
        if expected_iterations == 1:
            result["evidence_limit"] += (
                " The iteration-0 baseline and selection are the same checkpoint; this audit"
                " cannot establish parameter changes from unsaved pre-training initialization."
            )
...
def _plot_timing(rows: list[dict], output_dir: Path) -> None:
    plt = importlib.import_module("matplotlib.pyplot")
    iterations = [row["iteration"] for row in rows]
    with plt.style.context("seaborn-v0_8-whitegrid"):
        ...
        for axis, key, label in zip(
            axes.flat,
            SOURCE_TIMES,
            ("Worker env.step", "Parent request / response", "Wait for the last source response"),
        ):
            for name in SOURCE_ORDER:
                axis.plot(
                    iterations,
                    [row["sources"][name].get(key, float("nan")) for row in rows],
                    label=name,
                    linewidth=1,
                )
            ...
            axis.legend(fontsize=8)
        for key, label in (
            ("collect_seconds", "Global collection"),
            ("learn_seconds", "Shared PPO learner"),
        ):
            axes.flat[3].plot(iterations, [row[key] for row in rows], label=label)
        axes.flat[3].set_title("One global collection and PPO update")
        axes.flat[3].legend(fontsize=8)
        for axis in axes.flat:
            axis.set_xlabel("Completed iteration (zero based)")
            axis.set_ylabel("Seconds / iteration")
        fig.suptitle("Synchronous PPO timing · source durations overlap; do not add them")
        try:
            fig.savefig(output_dir / "timing.png", dpi=160)
            fig.savefig(output_dir / "timing.pdf")
        finally:
            plt.close(fig)
