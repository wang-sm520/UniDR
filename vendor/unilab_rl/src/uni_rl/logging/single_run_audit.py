...
def _run_metadata(run_dir: Path, iterations: int, num_envs: int) -> tuple[dict, str, Path]:
    ...
    samples = num_envs * 24 * iterations
    ...
    return algo, source, final_path


def _checkpoint_state(saved: dict, algo: dict, iterations: int, num_envs: int) -> float:
    """Validate saved native learner state at a completed iteration boundary."""
...
def _audit(run_dir: Path, iterations: int, num_envs: int) -> dict:
    algo, source, final_path = _run_metadata(run_dir, iterations, num_envs)
    saved = torch.load(final_path, map_location="cpu", weights_only=False)
    lr = _checkpoint_state(saved, algo, iterations, num_envs)
    samples, steps = num_envs * 24 * iterations, iterations * 20
    return {
        "audit_passed": True,
