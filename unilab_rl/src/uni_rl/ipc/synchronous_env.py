    ...
    try:
        ...
        while True:
            ...
            if operation == "init_state":
                ...
                result = _state_payload(env.step(payload))
...
class SynchronousEnv:
    ...
    def _exchange(self, operation: str | None, payloads: list[Any], timeout: float) -> list[Any]:
        if self._closed:
        ...
        if operation is not None:
            self._seq += 1
        try:
            futures = self._requests = [
                self._pool.submit(
                    _receive, conn, None if operation is None else (self._seq, operation, payload)
                )
                for conn, payload in zip(self._connections, payloads, strict=True)
            ]
            _, pending = wait(futures, timeout=timeout)
            if pending:
                missing = [
                    source.source_id
                    for source, future in zip(self.sources, futures, strict=True)
                    if future in pending
                ]
                raise TimeoutError(
                    f"{operation or 'startup'} request {self._seq}: missing {missing}"
                )
            values = []
            for source, future in zip(self.sources, futures, strict=True):
                try:
                    source_id, seq, status, value = future.result()
                except (EOFError, OSError) as exc:
                    raise RuntimeError(f"source {source.source_id} worker crashed") from exc
                if source_id != source.source_id or seq != self._seq:
    ...
    def step(self, actions: np.ndarray) -> _State:
        ...
        return self._aggregate(
            self._exchange(
                "step",
                [actions[self.source_slices[s.source_id]] for s in self.sources],
                self.timeout_s,
            ),
            transition=True,
        )
    ...
    def close(self) -> None:
        ...
        for index, conn in enumerate(self._connections):
            if not self._inflight or (index < len(self._requests) and self._requests[index].done()):
                self._pool.submit(conn.send, (self._seq + 1, "close", None))
