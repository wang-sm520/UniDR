...
TD3 runs through `src/unilab/scripts/train_td3.py` in its own config tree. Select it
with `--algo td3`; owner YAML evidence lives under `src/unilab/conf/td3/task/`.
...
For the off-policy playback path (`src/unilab/scripts/train_td3.py` / CLI `--algo td3`),
...
- Defaults are inlined in `src/unilab/conf/td3/config.yaml`.
