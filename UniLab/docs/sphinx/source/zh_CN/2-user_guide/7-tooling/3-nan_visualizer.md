...
PPO 在 `src/unilab/conf/ppo/config.yaml` 的 `training.nan_guard` 下设有一个 NaN guard，默认已启用，与 APPO/off-policy 一致。启用后，`src/unilab/scripts/train_rsl_rl.py` 会安装 `NanGuard`，检查观测字典和奖励，并在检测到 NaN/Inf 值时写入一个 `.npz` dump 以及模型元数据。
