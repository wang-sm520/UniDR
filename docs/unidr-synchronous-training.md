...
This is the 2026-09-16 implementation and experiment record. Historical source
paths, SHAs, commands explicitly labeled as executed, and results are preserved.
Operational examples now use the UniDR bundle; follow the [current README](../README.md)
for setup and the later physics fixes/adaptive interface. Server-side `logs/`
artifacts, checkpoints and videos referenced here are not part of this source release.
...
the server artifact `logs/unidr/stop-at5000-20260916/delivery.md`.
...
[runtime children and ADR](../vendor/unilab_rl/docs/unidr-central-ppo.md), and
...
preserved. At the time, these were local diffs, not published releases. AGENTS, licenses and locks
...
From the UniDR repository root after README setup, UV sources select the bundled
runtime and UniSim. No sibling checkout or `UNILAB_LOCAL_UNISIM` variable is needed:
...
To audit the historical user-selected earlier stopping point using this bundle,
first provide the original run artifacts and verify checkpoint/resource cleanup:

```bash
uv run --no-sync python vendor/unilab_rl/examples/report_synchronous.py logs/unidr/single-formal-1024-10000-20260916 logs/unidr/formal-report-5000-20260916 --expected-iterations 5001 --num-envs 1024 --final-checkpoint logs/unidr/single-formal-1024-10000-20260916/model_5000.pt
...
Historical gate commands executed with `UV_NO_SYNC=1` (original checkout paths;
`UNILAB_LOCAL_UNISIM` was used by that test profile and is not a current setup step):
...
After the fixed formal checkpoint exists, run from the UniDR repository root
with the completed run directory:
