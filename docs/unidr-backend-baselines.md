...
This is the historical September 2026 baseline and initial Motrix-alignment
record, not the current experiment recipe. Original paths, commands, dependency
versions and results below are preserved as evidence; `logs/` artifacts are not
included in the source bundle. For current setup and flip training, follow the
[repository README](../README.md) and [single-source comparison](unidr-single-comparison.md).
The bundle selects `vendor/unisim` and `vendor/unilab_rl` through its UV sources;
the historical `UNILAB_LOCAL_UNISIM` variable is not a current requirement and is
...
See the bundled [backend validation record](../vendor/unisim/docs/isaacgym-reset-validation.md).
Existing owner decisions are described
...
The original experiment checkout was `/home/wsm/wang-sm/UniLab-unidr-backends`:
...
Historical commands for the repository gates, from that checkout:
