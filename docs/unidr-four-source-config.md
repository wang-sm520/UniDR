...
This document records the shared configuration introduced in September 2026 and
its dated validation results. The operational commands use the bundled UniDR
layout; run them from the repository root after the [README setup](../README.md).
Historical `logs/` paths identify server artifacts, which are not included in git.
...
collector advances complete source pools according to their step quotas.
The source/rollout timeout is 180 seconds and
the startup timeout is 600 seconds.

## Focused Validation

From the UniDR repository root, using its bundled editable dependencies:
