...
set -euo pipefail
if [[ $# -lt 1 || $# -gt 5 ]]; then
  echo "Usage: $0 NEW_EXPERIMENT_ROOT [NUM_ENVS=1024] [ITERATIONS=20000] [SOURCE_ORDER_CSV] [--train-only]" >&2
  exit 2
fi
...
source_order=${4:-motrix,isaacsim,isaacgym,genesis}
holdout=true
if [[ $# -eq 5 ]]; then
  [[ $5 == --train-only ]] || exit 2
  holdout=false
fi
IFS=, read -r -a sources <<< "$source_order"
[[ ${#sources[@]} -eq 4 ]] || exit 2
declare -A seen=()
for source_name in "${sources[@]}"; do
  case "$source_name" in motrix|isaacsim|isaacgym|genesis) ;; *) exit 2 ;; esac
  [[ ! ${seen[$source_name]+present} ]] || exit 2
  seen[$source_name]=1
done
...
printf '{"num_envs":%s,"iterations":%s,"source_order":"%s","holdout":%s}\n' \
  "$num_envs" "$iterations" "$source_order" "$holdout" > "$experiment_root/queue_config.json"
source_name=none
stage=initializing
child=
completed=false
